...
while(True):
    ...
    #2. 决策变数的上下界
        ...
        target = [30,50]
        V_max = 10 #最大移动速度
        x = np.arange(X_LOWER[0],X_UPPER[0],1) #画网格用
        y = np.arange(X_LOWER[1],X_UPPER[1],1)
        #目标函数 能量函数E
        def E(params):
        ...
        X_LOWER = np.array([0,0]) #决策变量下界
        target = [4.71,4.71]
        V_max = float(input("最大速度：")) #最大移动速度
        x = np.arange(X_LOWER[0],X_UPPER[0],0.1) 
        y = np.arange(X_LOWER[1],X_UPPER[1],0.1)
        #目标函数 能量函数E
        def E(params):
    ...
    for k in range(Iter_final):
        ...
        plt.ion()
        plt.cla()
        ax3d.plot_surface(x,y,z,rstride=1,cstride=1,cmap=plt.cm.spring,alpha=0.7) #绘制曲面2
        ax3d.scatter(x_total[:,0],x_total[:,1],x_total[:,2]) #绘制粒子点
        ax3d.scatter(G[0],G[1],G[2],color='r')   #绘制当前的群体最佳解
